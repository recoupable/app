...
export function useArtistSocials() {
  ...
  const { data: socialsData } = useQuery({
    ...
    queryFn: () =>
      artist_account_id ? getArtistSocials(artist_account_id) : Promise.resolve(undefined),
    enabled: !!artist_account_id,
    staleTime: 1000 * 60 * 5,
  });

  const hasInstagram =
    socialsData?.socials?.some(s => s.profile_url?.toLowerCase().includes("instagram.com")) ??
    false;
