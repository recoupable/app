...
const receiptLink = (url: string, className: string) => (
  <a
    href={url}
    target="_blank"
...
/** One invoice. On a phone the status and receipt sit under the description; from md up each has a column. */
  ...
  return (
    <TableRow className="text-sm hover:bg-transparent">
      ...
      <TableCell className="w-full py-2.5 px-3">
        <div className="flex flex-col gap-1">
          ...
          <span className="flex items-center gap-3 md:hidden">
            {badge}
            {payment.url && receiptLink(payment.url, "md:hidden")}
          </span>
        </div>
      </TableCell>
      <TableCell className="hidden whitespace-nowrap py-2.5 px-3 md:table-cell">
        {badge}
      </TableCell>
      ...
      <TableCell className="hidden py-2.5 pl-3 pr-0 text-right md:table-cell">
        {payment.url && receiptLink(payment.url, "")}
      </TableCell>
    </TableRow>
  );
};
