...
import { getOrderedMessageParts } from "@/lib/vercel/getOrderedMessageParts";
...
export function MessageParts({ message, mode, setMode }: MessagePartsProps) {
  ...
  const lastTextOriginalIndex = orderedParts.reduce(
    (lastIndex, orderedPart) =>
      orderedPart.part.type === "text" ? orderedPart.originalIndex : lastIndex,
    -1,
  );

  return (
    <div className={cn("flex flex-col gap-4 w-full group")}>
      {orderedParts.map(({ originalIndex, part }) => {
        const { type } = part;
        ...
        if (type === "text") {
          const isLastMessage =
            ...
            status !== "streaming" &&
            originalIndex === lastTextOriginalIndex;
