...
import useFileMentionSuggestions, {
  GroupedSuggestion,
...
export default function FileMentionsInput({
  ...
  return (
    <MentionsInput
      ...
      disabled={disabled}
      className="w-full text-base leading-[1.6] pb-2 md:pb-0 text-foreground [&_textarea]:placeholder:text-muted-foreground"
      suggestionsPortalHost={portalHost}
      allowSuggestionsAboveCursor
      customSuggestionsContainer={(children) => (
        <Card
          className="z-[70] shadow-lg border border-border rounded-xl overflow-hidden p-1 max-w-32 bg-popover"
    ...
    >
      <Mention
        ...
        renderSuggestion={(
          ...
          const prev =
            index > 0
              ? (lastResults[index - 1] as GroupedSuggestion | undefined)
              : undefined;
          const showHeader =
            !prev || (current && prev && current.group !== prev.group);
