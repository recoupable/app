...
[hidden]{display:none!important}body[data-preview="false"] header{display:none}.entrance{position:fixed;inset:0;z-index:20;background:rgb(255 255 255 / 32%);backdrop-filter:blur(4px)}.music-panel{position:fixed;right:39px;top:14px;width:375px;z-index:30;border-radius:12px;overflow:hidden;box-shadow:#dcdcdc 0 0 0 .5px inset,0 6px 10px #00000026;background:white;color:#1f1a1e}.music-panel iframe{height:260px;min-height:0;width:100%;border:0}.music-panel.collapsed{visibility:hidden;pointer-events:none;transform:translateX(calc(100% + 40px))}.music-toggle{position:fixed;right:0;top:50px;z-index:31;background:black;color:white;width:58px;height:52px;border-radius:10px 0 0 10px;padding:5px;display:flex;align-items:center;justify-content:center}.music-toggle span{display:flex;align-items:center;justify-content:center;background:#d9d9d940;width:45px;height:42px;border-radius:5px}.music-panel:not(.collapsed)~.music-toggle{display:none}#experience[inert]{pointer-events:none}@media(max-width:640px){.music-panel{top:auto;bottom:0;right:0;width:100%;border-radius:12px 12px 0 0}.music-panel.collapsed{transform:translateY(100%)}}
body:not([data-entered]) .music-panel{top:50%;left:50%;right:auto;bottom:auto;width:min(375px,calc(100% - 32px));border-radius:12px;transform:translate(-50%,-50%)}
</style></head><body data-sites-runtime data-audio-url="${esc(site.assets.find((asset) => asset.type === "audio")?.url || "")}" data-connect-url="${esc(connectUrl || "")}" data-preview="${!signupAction}" data-release="${esc(site.releaseUrl)}"><header><strong>${esc(site.name)}</strong><div class="controls"><button id="spotify-connect" type="button">Connect Spotify</button><button id="spotify-play" type="button" hidden>Play music</button><button id="spotify-disconnect" type="button" hidden>Disconnect</button><span id="spotify-track" hidden><img id="spotify-cover" width="40" height="40" alt=""><a id="spotify-track-link" target="_blank" rel="noopener noreferrer"></a></span><span id="spotify-status" role="status">${signupAction ? "Connect to listen while you play." : connectUrl ? "Test Spotify in a separate window. Your draft stays private." : "Spotify connects on your published site."}</span>${site.releaseUrl ? `<a href="${esc(site.releaseUrl)}" target="_blank" rel="noopener noreferrer">Open music ↗</a>` : ""}</div></header><iframe id="experience" title="${esc(site.name)} experience" sandbox="allow-scripts" srcdoc="${esc(child)}"></iframe>${signupAction ? `<div class="entrance" id="music-entrance"></div><aside class="music-panel" id="music-panel" aria-label="Music player"><iframe id="music-frame" title="Spotify player" allow="autoplay *; encrypted-media *" src="about:blank"></iframe></aside><button id="music-toggle" class="music-toggle" type="button" hidden aria-expanded="true" aria-label="Open music player"><span><img src="/syncstream/minimize.png" width="34" height="32" alt=""></span></button>` : ""}<footer>${site.assets
