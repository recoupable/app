...
export function renderSpotifyPlayer(
  ...
  const accentRgb = theme
    ? [1, 3, 5]
        .map(
          (offset) =>
            parseInt(theme.accent.slice(offset, offset + 2), 16) / 255,
        )
        .map((c) => (c <= 0.04045 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4))
    : [1, 1, 1];
  const accentText =
    accentRgb[0] * 0.2126 + accentRgb[1] * 0.7152 + accentRgb[2] * 0.0722 >
    0.179
      ? "#000000"
      : "#ffffff";
  const themeStyle = theme
    ? `<style>:root{--release-background:${theme.background};--release-foreground:${theme.foreground};--release-accent:${theme.accent};--release-accent-text:${accentText};--release-font:${theme.font === "serif" ? "Georgia,serif" : "Circular,Arial,sans-serif"}}</style>`
    : "";
  const artwork = theme?.artwork
    ? `<img class="release-cover" src="${escape(theme.artwork)}" alt="${escape(theme.title)} artwork">`
    : "";
  ...
  <section class="sign-body">${artwork}<h1>${theme ? escape(theme.title) : "Music, while you play."}</h1><p id="spotify-intro" class="sign-description">Connect Spotify to listen in the game.</p><button id="spotify-connect" class="sign-button">Connect Spotify</button><button id="spotify-continue" class="sign-button" hidden>Enter game</button><p class="terms"><span class="consent-copy">Recoup receives your Spotify profile and email.</span> <a href="https://recoupable.dev/privacy" target="_blank" rel="noopener noreferrer">Privacy</a> · <a href="https://recoupable.dev/terms" target="_blank" rel="noopener noreferrer">Terms</a>.</p></section>
