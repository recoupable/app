...
export function renderSpotifyPlayer(
  ...
  <header class="sign-header"><span><img src="/syncstream/spotify.svg" alt="Spotify" width="20" height="20">Sign in to Recoup with Spotify</span><button id="spotify-minimize" class="close" aria-label="Minimize player">${icon("M6.4 5 12 10.6 17.6 5 19 6.4 13.4 12 19 17.6 17.6 19 12 13.4 6.4 19 5 17.6 10.6 12 5 6.4Z")}</button></header>
  <section class="sign-body"><button id="spotify-connect" class="sign-button">Continue to Game</button><button id="spotify-continue" class="sign-button" hidden>Continue to Game</button><p class="terms">Spotify will share your profile and email with Recoup. <a href="https://recoupable.dev/privacy" target="_blank" rel="noopener noreferrer">Privacy policy</a> · <a href="https://recoupable.dev/terms" target="_blank" rel="noopener noreferrer">Terms of service</a>.</p></section>
  ...
  <p id="spotify-status" role="status">Spotify Premium is required for browser playback.</p><footer><button id="spotify-skip">Continue without music</button><a href="${release}" target="_blank" rel="noopener noreferrer">Open in Spotify</a><button id="spotify-disconnect" hidden>Disconnect</button></footer>
