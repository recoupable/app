...
/**
 * The weekly streaming-report prompt used both for the onboarding pre-run
 * (fired through the normal chat pipeline) and as the scheduled task's
 * prompt — one string so the report the user previews is exactly what the
 * Monday run emails (chat#1867, first-task step).
 *
 ...
 * sandbox/no-python and anti-fabrication guardrails are carried over
 * verbatim because they are what make the send reliable.
 */
...
4. Also GET /api/spotify/artist?id={spotify_artist_id} and keep the artist's largest image URL for the email header.
...
- Audience Snapshot: current Spotify monthly listeners, followers, and playlist reach (endpoint 8). Monthly-listener week-over-week change is not measured; report the current value only and do not estimate it.
...
- Header: full-width artist photo at the top (the largest image from step 4), width 600px, height auto, then a 4px solid #111111 bar, then the title "${artistName} - Weekly Streaming Report" in bold 22px #111111 with the date range in 13px #6B7280 underneath.
...
Email subject: ${artistName} - Weekly Streaming Report. Never fabricate a number: every figure must come from one of the API responses above. If a call fails or returns no data, say so in the report instead of estimating.`;
}
